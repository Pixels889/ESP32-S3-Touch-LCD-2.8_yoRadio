...
class TextWidget: public Widget {
  public:
    TextWidget() {}
    // 修改：增加字体参数
    TextWidget(WidgetConfig wconf, uint16_t buffsize, bool uppercase, const uint8_t* font, uint16_t fgcolor, uint16_t bgcolor) {
      init(wconf, buffsize, uppercase, font, fgcolor, bgcolor);
    }
    ~TextWidget();
    using Widget::init;
    // 修改：增加字体参数
    void init(WidgetConfig wconf, uint16_t buffsize, bool uppercase, const uint8_t* font, uint16_t fgcolor, uint16_t bgcolor);
...
class ScrollWidget: public TextWidget {
  public:
    ScrollWidget(){}
    // 修改：构造函数增加字体参数
    ScrollWidget(const char* separator, ScrollConfig conf, const uint8_t* font, uint16_t fgcolor, uint16_t bgcolor);
    ~ScrollWidget();
    using Widget::init;
    // 修改：init增加字体参数
    void init(const char* separator, ScrollConfig conf, const uint8_t* font, uint16_t fgcolor, uint16_t bgcolor);
    void loop();
    void setText(const char* txt);
    void setText(const char* txt, const char *format);
  private:
    char *_sep;
    char *_window;
    int16_t _pixelOffset;   // <--- 新增：连续滚动总偏移量
    int16_t _x;
    bool _doscroll;
    ...
    uint8_t _charWidth;
    uint16_t _u8g2CharWidth;  // U8g2实际字体宽度
...
class NumWidget: public TextWidget {
  public:
    using Widget::init;
    // NumWidget 不修改字体参数，仍使用原有接口（调用父类时传入 nullptr 或默认字体）
...
class ProgressWidget: public TextWidget {
  public:
    ProgressWidget() {}
    // 修改：init增加字体参数，并传递给父类
    ProgressWidget(WidgetConfig conf, ProgressConfig pconf, const uint8_t* font, uint16_t fgcolor, uint16_t bgcolor) { 
      init(conf, pconf, font, fgcolor, bgcolor);
    }
    using Widget::init;
    void init(WidgetConfig conf, ProgressConfig pconf, const uint8_t* font, uint16_t fgcolor, uint16_t bgcolor){
      TextWidget::init(conf, pconf.width, false, font, fgcolor, bgcolor);
