...
#ifndef MIN_BRIGHTNESS_PWM
#define MIN_BRIGHTNESS_PWM 5   // 最低亮度对应的 PWM 值（0-255），可根据实际硬件调整
#endif
...
class Config {
  public:
    ...
    station_t station;
    theme_t   theme;
    uint8_t getBrightnessPWM()
    {
      // 将 brightness (0-100) 映射到 MIN_BRIGHTNESS_PWM ~ 255
      return map(store.brightness, 0, 100, MIN_BRIGHTNESS_PWM, 255);
    }
#if IR_PIN!=255
    int irindex;
    uint8_t irchck;
