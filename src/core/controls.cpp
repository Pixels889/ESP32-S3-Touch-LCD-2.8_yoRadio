...
#ifdef USE_QMI8658
void initQMI8658() {
    QMI8658_Init();  // 该函数内已经包含校准和配置
}

// 动作识别阈值，可根据实际情况调整
#define ACCEL_SHAKE_THRESHOLD  3.5f   // 加速度变化阈值 (g)
#define GYRO_TWIST_THRESHOLD   300.0f // 角速度变化阈值 (°/s)
#define SHAKE_COOLDOWN_MS       300   // 同方向动作冷却时间 (ms)

static uint32_t lastShakeTime[6] = {0}; // 分别对应前、后、左、右、上、下

void loopQMI8658() {
    // 1. 更新传感器数据（驱动内部已更新 Accel 和 Gyro 全局变量）
    QMI8658_Loop();  // 这个函数会更新 Accel, Gyro 和姿态角

    // 2. 获取当前时间
    uint32_t now = millis();

    // 3. 动作识别（基于加速度峰值，忽略小运动）
    // 前甩：加速度 Y 轴正向大峰值（假设屏幕朝上，Y轴向前）
    if (Accel.y > ACCEL_SHAKE_THRESHOLD && now - lastShakeTime[0] > SHAKE_COOLDOWN_MS) {
        lastShakeTime[0] = now;
        // 音量加40
        int newVol = config.store.volume + 40;
        if (newVol > 254) newVol = 254;
        player.setVol(newVol);
        return;
    }
    // 后甩：加速度 Y 轴负向大峰值
    if (Accel.y < -ACCEL_SHAKE_THRESHOLD && now - lastShakeTime[1] > SHAKE_COOLDOWN_MS) {
        lastShakeTime[1] = now;
        int newVol = config.store.volume - 40;
    ...
    // 右甩：绕 Z 轴角速度正向大峰值（右转）
    if (Gyro.z > GYRO_TWIST_THRESHOLD && now - lastShakeTime[2] > SHAKE_COOLDOWN_MS) {
        lastShakeTime[2] = now;
        player.next();   // 列表下一个
        return;
    }
    // 左甩：绕 Z 轴角速度负向大峰值
    if (Gyro.z < -GYRO_TWIST_THRESHOLD && now - lastShakeTime[3] > SHAKE_COOLDOWN_MS) {
        lastShakeTime[3] = now;
        player.prev();   // 列表上一个
        return;
    }
    // 向上甩：加速度 Z 轴正向大峰值（假设设备平放，向上拿起）
    if (Accel.z > ACCEL_SHAKE_THRESHOLD && now - lastShakeTime[4] > SHAKE_COOLDOWN_MS) {
        lastShakeTime[4] = now;
        // 切换到电台模式（如果当前不是电台）
    ...
    // 向下甩：加速度 Z 轴负向大峰值
    if (Accel.z < -ACCEL_SHAKE_THRESHOLD && now - lastShakeTime[5] > SHAKE_COOLDOWN_MS) {
        lastShakeTime[5] = now;
        // 切换到 SD 卡模式（如果当前不是 SD 卡）
