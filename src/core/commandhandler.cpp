...
bool CommandHandler::exec(const char *command, const char *value, uint8_t cid) {
  ...
  if (strEquals(command, "flipscreen"))   { config.saveValue(&config.store.flipscreen, static_cast<bool>(atoi(value))); config.saveValue(&config.store.fliptouch, static_cast<bool>(atoi(value))); display.flip(); flipTS(); display.putRequest(NEWMODE, CLEAR); display.putRequest(NEWMODE, PLAYER); return true; }
