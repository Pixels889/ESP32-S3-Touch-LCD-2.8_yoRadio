...
void MyNetwork::raiseSoftAP() {
  ...
  Serial.println("Entering AP mode loop...");
  ...
  while (true) {
    ...
    if (millis() - lastFeed > 2000) {
      esp_task_wdt_reset();
      lastFeed = millis();
      Serial.printf("Feeding watchdog in AP mode... (loop: %lu)\n", loopCount);
    }
    
    // 处理Web请求，让配置页面能响应
    netserver.loop();
    
    // 检查是否已经配置完成（有WiFi配置且SPIFFS不为空）
    if (!config.emptyFS && config.ssidsCount > 0) {
      Serial.println("Configuration detected, exiting AP mode");
      // 退出前多喂几次狗，确保后续初始化有足够时间
      for (int i = 0; i < 10; i++) {
          esp_task_wdt_reset();
          delay(50);
          }
